...
本地修改代码完成后，必须执行部署流程让远程服务器更新。

### 快速部署 (推荐)

```bash
./deploy.sh "fix: your commit message"   # 指定消息
./deploy.sh                              # 自动根据修改文件生成消息
```

deploy.sh 流程: `git add -A` → `git commit` → `git push origin main` → `ssh rack` 远程更新。
...
2. SSH 到 rack 服务器执行更新
3. 确认服务正常运行 (`curl http://107.174.67.124:3001/api/health`)

## 服务器管理 (rack)

```bash
# SSH 连接 (始终使用 CC 用户)
ssh CC@rack              # 方式1: 直接以 CC 用户 SSH
ssh rack && su - CC      # 方式2: 从 root 切换到 CC
...
# 手动更新
cd ~/CCManager && git pull && pnpm run build && pm2 restart ccm-server
...
服务器端环境变量 (已配置在服务器 ecosystem.config.cjs 中):
